...
def get_supabase():
    try:
        ...
        # Fallback to hardcoded keys if secrets are missing
...
def init_new_chat():
    ...
    return {
        nid: {
            "title": "New Chat", 
            "messages": [{"role": "assistant", "content": "Hey! I'm Kyle AI - AS Level 9618/9709/9702/9231 expert. Ask me anything!"}]
        }
    }, nid

if "chats" not in st.session_state or st.session_state.get("loaded_uid") != uid:
    loaded = load_chats_for_user(uid)
    if loaded:
        st.session_state.chats = loaded
        st.session_state.current_chat = list(loaded.keys())[0]
    else:
        st.session_state.chats, st.session_state.current_chat = init_new_chat()
...
# --- GROQ SETUP ---
TEXT_MODELS = ["llama-3.3-70b-versatile", "llama-3.1-8b-instant", "mixtral-8x7b-32768"]
...
def call_groq_auto(messages, max_tokens):
    for mid in TEXT_MODELS:
        try: 
            ...
            continue
    raise Exception("All configured Groq models failed. Please try again.")

# --- RAG / BRAIN LOGIC ---
@st.cache_resource
def load_brain(_hash):
    if not os.path.exists("chunks.pkl") or not os.path.exists("model.faiss"):
        raise FileNotFoundError("Missing chunks.pkl or model.faiss")
...
try:
    file_hash = os.path.getmtime("chunks.pkl") if os.path.exists("chunks.pkl") else 0
    chunks, index, embed_model = load_brain(file_hash)
except Exception as e:
    st.error(f"Brain files missing or corrupted: {e}")
    st.stop()

# --- FORMATTING & PROMPT LOGIC ---
def fix(t):
    def clean_sql(m):
        inner = m.group(1)
...
with st.sidebar:
    ...
    if st.button("➕ New Chat", use_container_width=True, type="primary"):
        new_chats, new_id = init_new_chat()
        st.session_state.chats[new_id] = new_chats[new_id]
...
# --- MAIN CHAT INTERFACE ---
# Safely handle missing current chat
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        # Excludes the prompt that was just appended to grab prior context only
        history = current["messages"][-10:-1] 
        ...
        for txt in texts:
            if any(a2 in txt.lower() for a2 in ["p3", "paper 3", "m2", "s2", "a2 level"]): 
                continue
            if any(k in txt for k in ["M1", "A1", "B1", "M0", "FT"]): 
                ms.append(txt)
            else: 
                qp.append(txt)
        ...
        if is_greeting(prompt):
            msgs = [
                {"role": "system", "content": "You are Kyle AI. Greeting only, friendly short. AS Level only."}, 
            ...
            # Add memory messages for proper conversational context
            for m in history[-6:]:
